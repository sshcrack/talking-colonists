...
| Housing | `CitizenWellbeingView.homeless`; factory uses `getHomeBuilding() == null` | A low-level but assigned residence can be described as poor housing while old recollection says “no home”; the boolean model cannot express unknown. | `HomeCheck.promptLine` scans colonies by citizen name and emits a “HOME TRUTH” block. | Explicit `HOUSED`, `HOMELESS`, `UNKNOWN` housing state sourced from the MineColonies home assignment only; a guard workplace is not housing. |
